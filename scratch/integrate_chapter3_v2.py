    ...
    try:
        ...
        for filename, caption_text in uc_files:
            ...
            if os.path.exists(path):
                ...
                p = doc.add_paragraph()
                p.add_run(f"Figure {fig_counter}: {caption_text}.").bold = True
        ...
        if os.path.exists(class_diag):
            ...
            p = doc.add_paragraph()
            p.add_run(f"Figure {fig_counter}: diagramme de classes du système FaceAttend.").bold = True
            p.alignment = 1
            fig_counter += 1
        ...
        print("Mise à jour réussie : Le chapitre III complet a été ajouté au document avec le formatage des figures corrigé !")
