...
def generate_standalone_chapter():
    output_path = "/home/usain/Bureau/FaceAttend/docs/Chapitre_III_Complet_Officiel.docx"
    ...
    try:
        ...
        for filename, caption_text in uc_files:
            ...
            if os.path.exists(path):
                if fig_counter > 1:
                    doc.add_page_break() # Règle : 1 figure max par page (pas de saut avant la toute première figure)
                
                # Image encadrée (le cadre est déjà généré dans le PNG par PIL) et centrée
                p_img = doc.add_paragraph()
                p_img.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p_img.add_run().add_picture(path, width=Inches(6.2))
                
                # Légende (Format : Figure n° x : Titre)
                p_caption = doc.add_paragraph()
                p_caption.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p_caption.add_run(f"Figure n° {fig_counter} : {caption_text}").bold = True
                
                # Source (Format : Source : L'auteur, taille 8)
                p_source = doc.add_paragraph()
                p_source.alignment = WD_ALIGN_PARAGRAPH.CENTER
                run_source = p_source.add_run("Source : L'auteur")
        ...
        for filename, caption_text in seq_files:
            ...
            if os.path.exists(path):
                doc.add_page_break() # Règle : 1 figure max par page
        ...
        if os.path.exists(class_diag):
            ...
            p_img = doc.add_paragraph()
            p_img.alignment = WD_ALIGN_PARAGRAPH.CENTER
            p_img.add_run().add_picture(class_diag, width=Inches(6.2))
            
            p_caption = doc.add_paragraph()
            p_caption.alignment = WD_ALIGN_PARAGRAPH.CENTER
            p_caption.add_run(f"Figure n° {fig_counter} : diagramme de classes du système FaceAttend.").bold = True
