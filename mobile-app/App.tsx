      ...
      <SafeAreaView style={styles.loginContainer}>
        ...
        <View style={styles.loginCard}>
          ...
          <TouchableOpacity 
            ...
            disabled={loading}
          >
            {loading ? <ActivityIndicator color="#020617" /> : <Text style={styles.loginButtonText}>SE CONNECTER</Text>}
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
           ...
           <View style={styles.alertSection}>
              ...
              {alerts.map(a => (
                <View key={a.id} style={styles.alertItem}>
                   ...
                   <Text style={styles.alertStatus}>{a.absence_count} absences / {a.threshold} max</Text>
                </View>
              ))}
           </View>
         )}
           ...
           renderItem={({ item }) => (
             <View style={styles.recordItem}>
                <View>
                  ...
                  <Text style={styles.recordDate}>{new Date(item.marked_at).toLocaleDateString('fr-FR')}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  loginCard: { backgroundColor: '#0f172a', padding: 40, borderRadius: 30, borderWeight: 1, borderColor: '#1e293b' },
  loginLogo: { color: '#00f0ff', fontSize: 32, fontWeight: '900', textAlign: 'center' },
  loginTagline: { color: '#64748b', fontSize: 12, fontWeight: 'bold', textAlign: 'center', letterSpacing: 5, marginBottom: 40 },
  inputGroup: { marginBottom: 25 },
  label: { color: '#64748b', fontSize: 10, fontWeight: 'black', marginBottom: 10, letterSpacing: 1 },
  input: { backgroundColor: '#1e293b', color: '#fff', padding: 18, borderRadius: 12, fontSize: 16 },
  loginButton: { backgroundColor: '#00f0ff', padding: 20, borderRadius: 15, alignItems: 'center', marginTop: 10 },
  loginButtonText: { color: '#020617', fontWeight: '900', letterSpacing: 2 },
  
  // Dashboard
  container: { flex: 1, backgroundColor: '#020617' },
  header: { padding: 30, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#1e293b' },
  ...
  alertItem: { backgroundColor: '#450a0a', padding: 15, borderRadius: 12, marginBottom: 10, borderLeftWidth: 4, borderLeftColor: '#ef4444' },
  alertCourse: { color: '#fff', fontWeight: 'bold' },
  alertStatus: { color: '#fca5a5', fontSize: 12 },
  recordItem: { backgroundColor: '#0f172a', padding: 15, borderRadius: 12, marginBottom: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
