...
import { Search, User, ShieldCheck, Activity, ArrowLeft, Check, AlertCircle, Loader2, ImagePlus, QrCode, X } from 'lucide-react';
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white rounded-[3rem] p-10 max-w-sm w-full shadow-2xl relative animate-in zoom-in-95 duration-300">
              ...
              <div className="text-center space-y-6">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Accès Mobile</h3>
                  <p className="text-slate-500 text-sm mt-1">Configurez l'IP pour votre téléphone</p>
                </div>

                <div className="space-y-2 text-left">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Adresse IP du PC</label>
                  <input 
                    type="text"
                    value={deviceIP}
                  ...
                  <p className="text-[9px] text-slate-400 italic px-1 leading-relaxed">
                    Tapez 'ipconfig' (Windows) ou 'ifconfig' (Linux) dans un terminal pour trouver votre IP Wi-Fi.
                  </p>
                </div>
          ...
          <div className="space-y-6 sticky top-6">
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Code Matricule (Password)</label>
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Email (Identifiant)</label>
                <div className="flex gap-2">
                  <input
                    type="email"
                    value={editEmail}
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Photo d'Identité</label>
                <input
                  type="file"
                  ref={fileInputRef}
                  className="hidden"
                ...
                >
                  {isUpdating ? <Loader2 size={16} className="animate-spin" /> : <ImagePlus size={16} />}
                  Changer la photo
                </button>
              </div>
              ...
              <div className="min-h-[40px]">
                {status && (
                  <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest p-3 rounded-xl animate-in slide-in-from-top-1 ${status.type === 'success' ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50'
                    }`}>
              ...
              <div className="pt-4 border-t border-slate-100 space-y-3">
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] border transition-all ${currentStudent.is_active
                ...
                <button
                  onClick={async () => {
                    ...
                    const res = await fetch(`${API}/api/students/${selectedStudentId}`, { method: 'DELETE' });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Simulation Étudiant</h2>
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Audit et vérification des comptes</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="flex items-center gap-5 relative z-10">
                <div className="w-14 h-14 rounded-2xl border-2 border-slate-100 overflow-hidden bg-slate-50 shrink-0">
                  <img
                    src={getPhotoUrl(student.photo_url) || `https://ui-avatars.com/api/?name=${encodeURIComponent(student.full_name)}&background=10b981&color=fff`}
              ...
              <div className="mt-6 pt-5 border-t border-slate-50 flex items-center justify-between">
                <div className="flex items-center gap-2 text-slate-400 text-[9px] font-bold uppercase tracking-widest">
                  <Activity size={12} className={student.is_active ? "text-emerald-500" : "text-amber-500"} />
          ...
          {filteredStudents.length === 0 && (
            <div className="col-span-full py-20 text-center bg-white rounded-[2.5rem] border border-dashed border-slate-300">
              ...
              <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">Aucun étudiant trouvé</p>
            </div>
          )}
        </div>
      )}
