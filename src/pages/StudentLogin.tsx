...
export default function StudentLogin({ onLogin, theme: currentTheme }: StudentLoginProps) {
  ...
  const theme = {
    container: isDark ? 'bg-[#020617]' : 'bg-slate-50',
    card: isDark ? 'bg-[#0b1219] border-slate-800 shadow-2xl' : 'bg-white border-slate-200 shadow-xl',
    ...
    buttonText: isDark ? 'text-slate-950' : 'text-white',
    title: 'Portrait Étudiant',
    subtitle: 'Accès Portail Académique',
    iconBg: isDark ? 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.15)]' : 'bg-emerald-50 border-emerald-100 shadow-sm',
  ...
  return (
    <div
      className="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-white flex items-center justify-center p-4 font-sans selection:bg-white/20 relative overflow-hidden"
    >
      {/* ─── BACKGROUND EFFECTS ─── */}
      <div className="absolute inset-0 pointer-events-none transition-opacity duration-1000">
        ...
        <div
          ...
          style={{ backgroundImage: theme.gridLines, backgroundSize: '40px 40px', opacity: isDark ? 0.3 : 0.1 }}
        />
      </div>
      ...
      <div className="w-full max-w-md relative animate-in fade-in zoom-in-95 duration-700 ease-out z-10">
        ...
        <div className={`relative ${theme.card} border rounded-3xl p-8`}>
          ...
          <div className="flex flex-col items-center mb-8 animate-in slide-in-from-top-4 fade-in duration-700 delay-100 fill-mode-both">
            ...
            <h1 className={`text-2xl font-bold ${theme.textColor} tracking-tight text-center`}>
              {theme.title}
            </h1>
            <p className={`${theme.mutedText} text-sm mt-1.5 font-medium tracking-wide`}>
              {theme.subtitle}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-300 fill-mode-both">
              <label className={`block text-xs font-semibold ${theme.mutedText} uppercase tracking-wider mb-2 pl-1`}>
                Email Professionnel
              </label>
              <div className="relative group">
                ...
                <input
                  ...
                  className={`block w-full pl-11 pr-4 py-3.5 ${isDark ? 'bg-black/40' : 'bg-slate-100'} border ${isDark ? 'border-slate-700/50' : 'border-slate-200'} ${theme.textColor} rounded-xl ${theme.inputFocus} transition-all duration-300 placeholder:text-slate-600 outline-none`}
            ...
            <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-400 fill-mode-both">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`block w-full pl-11 pr-12 py-3.5 ${isDark ? 'bg-black/40' : 'bg-slate-100'} border ${isDark ? 'border-slate-700/50' : 'border-slate-200'} ${theme.textColor} rounded-xl ${theme.inputFocus} transition-all duration-300 placeholder:text-slate-600 outline-none tracking-widest`}
                  placeholder="••••••••"
                  required
                />
                <button
                  ...
                  className={`absolute inset-y-0 right-0 pr-4 flex items-center ${theme.mutedText} hover:${theme.textColor} transition-colors`}
                >
          ...
          <div className={`mt-8 pt-6 border-t ${isDark ? 'border-slate-700/50' : 'border-slate-200'} text-center animate-in fade-in duration-700 delay-700 fill-mode-both`}>
            <p className={`${theme.mutedText} text-xs font-medium`}>
              Système sécurisé par reconnaissance faciale
            </p>
            <div className={`mt-4 px-4 py-2 ${isDark ? 'bg-white/5' : 'bg-slate-200/50'} rounded-lg inline-flex items-center gap-2 border ${isDark ? 'border-white/5' : 'border-slate-300/30'}`}>
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className={`text-[10px] ${theme.mutedText} font-bold uppercase tracking-widest`}>
