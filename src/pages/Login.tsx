...
export default function Login({ onLogin, forceRole }: LoginProps) {
  ...
  const theme = {
    ...
    inputFocus: isTeacher ? 'focus:ring-amber-500/30 focus:border-amber-500' : 'focus:ring-[#00f0ff]/30 focus:border-[#00f0ff]',
    ...
    iconBg: isTeacher ? 'bg-amber-500/10 border-amber-500/20 shadow-[0_0_20px_rgba(245,158,11,0.15)]' : 'bg-[#00f0ff]/10 border-[#00f0ff]/20 shadow-[0_0_20px_rgba(0,240,255,0.15)]'
  };

  return (
    <div className={`min-h-screen ${theme.container} flex items-center justify-center p-4 font-sans selection:bg-white/20 transition-colors duration-700 relative overflow-hidden`}>

      {/* ─── BACKGROUND EFFECTS ─── */}
      <div className="absolute inset-0 pointer-events-none transition-opacity duration-1000">
        <div className={`absolute top-[-10%] left-[-10%] w-[50%] h-[50%] ${theme.blob1} blur-[140px] rounded-full transition-colors duration-1000`} />
        <div className={`absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] ${theme.blob2} blur-[140px] rounded-full transition-colors duration-1000`} />
        <div
      ...
      <div className="w-full max-w-md relative animate-in fade-in zoom-in-95 duration-700 ease-out z-10">
        {/* Glow behind card */}
        <div className={`absolute -inset-1 bg-gradient-to-r ${theme.glowBg} rounded-[2rem] blur-lg opacity-20 group-hover:opacity-40 transition duration-1000`}></div>

        <div className={`relative ${theme.card} border rounded-3xl p-8 transition-colors duration-700`}>
          ...
          <div className="flex flex-col items-center mb-8 animate-in slide-in-from-top-4 fade-in duration-700 delay-100 fill-mode-both">
            <div className={`w-20 h-20 rounded-2xl flex items-center justify-center border mb-5 transition-all duration-700 ${theme.iconBg}`}>
              {isTeacher ? <GraduationCap size={40} className={theme.iconColor} /> : <ShieldCheck size={40} className={theme.iconColor} />}
            </div>
          ...
          {!forceRole && (
            <div className="flex gap-2 mb-8 p-1.5 bg-black/40 border border-white/5 rounded-xl animate-in fade-in duration-700 delay-200 fill-mode-both">
              <button
                ...
                className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-all duration-300 ${!isTeacher
