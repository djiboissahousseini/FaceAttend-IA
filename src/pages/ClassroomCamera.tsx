...
import { ShieldAlert, UserCheck, UserX, AlertTriangle, Activity, ScanFace, Lock, Fingerprint, Video } from 'lucide-react';
import { Session } from '../types';

type ScanStatus = 'standby' | 'ready_teacher' | 'holding_teacher' | 'scanning_teacher' | 'teacher_success' | 'teacher_error' | 'ready' | 'holding' | 'scanning' | 'success' | 'duplicate' | 'wrong_group' | 'unknown' | 'error';
...
export default function ClassroomCamera() {
  ...
  const captureAndScan = async (targetType: 'teacher' | 'student') => {
    ...
    if (!image) {
      handleResult(targetType === 'teacher' ? 'teacher_error' : 'error', { message: 'ERREUR CAMÉRA' });
      return;
    }
    ...
    try {
      ...
      if (targetType === 'teacher') {
         ...
         if (data.status === 'success') {
           ...
           handleResult('wrong_group', { name: data.student?.name, message: data.message || 'GROUPE INVALIDE' });
         } else {
           handleResult('unknown', { message: 'VISAGE INCONNU' });
         }
      }
    } catch (e) {
      handleResult(targetType === 'teacher' ? 'teacher_error' : 'error', { message: 'ERREUR RÉSEAU' });
    }
  };
            ...
            case 'FORCE_START_SESSION':
              const fetchSessionToForce = async () => {
                try {
                  ...
                  const target = sessions.find(s => Number(s.id) === Number(data.payload.session_id));
                  if (target) {
  ...
  const themeClass = isTeacherMode ? 'text-[#ff003c] border-[#ff003c]' : 'text-[#00f0ff] border-[#00f0ff]';
    ...
    <div className="absolute inset-0 z-40 bg-[#020617] flex flex-col items-center justify-center font-mono overflow-hidden select-none">
      ...
      <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-50 bg-gradient-to-b from-black/80 to-transparent backdrop-blur-sm">
        <div className="flex items-center gap-4">
          <div className={`p-2 border rounded-none ${themeClass} bg-black/50`} style={{ boxShadow: `0 0 15px ${themeColor}` }}>
            <Activity size={28} className="animate-pulse" />
          </div>
          <div>
            <div className={`font-bold text-2xl uppercase tracking-[0.2em] ${themeClass.split(' ')[0]} drop-shadow-[0_0_8px_CURRENTCOLOR]`}>
              {classroom}
            </div>
            <p className="text-slate-400 text-xs tracking-widest mt-1">SYSTÈME DE PRÉSENCE INTELLIGENT</p>
          </div>
        </div>
        ...
        <div className="text-right flex flex-col items-end">
          ...
          <div className={`${themeClass.split(' ')[0]} text-xs tracking-widest mt-1 uppercase transition-colors duration-500`}>
      ...
      <div className="relative w-full max-w-5xl aspect-video z-20 mt-8 bg-black flex items-center justify-center border border-slate-800/50 shadow-[0_0_50px_rgba(0,0,0,1)] rounded-xl overflow-hidden">
        ...
        {/* Physical Corners */}
        <div className={`absolute top-0 left-0 w-12 h-12 border-t-4 border-l-4 ${themeClass} z-30 transition-colors duration-500`} />
        <div className={`absolute top-0 right-0 w-12 h-12 border-t-4 border-r-4 ${themeClass} z-30 transition-colors duration-500`} />
        <div className={`absolute bottom-0 left-0 w-12 h-12 border-b-4 border-l-4 ${themeClass} z-30 transition-colors duration-500`} />
        <div className={`absolute bottom-0 right-0 w-12 h-12 border-b-4 border-r-4 ${themeClass} z-30 transition-colors duration-500`} />
        ...
        {status === 'standby' ? (
          <div className="w-full h-full flex flex-col items-center justify-center bg-[#020617] text-[#00f0ff]/40">
             ...
             <h2 className="text-3xl font-bold tracking-[0.2em] uppercase mb-3 text-[#00f0ff]/60">VEILLE SYSTÈME</h2>
             <p className="tracking-[0.1em] text-sm text-slate-500">AUCUN COURS DÉTECTÉ POUR LE MOMENT</p>
          </div>
        ) : (
            ...
            {(status.includes('holding') || status.includes('scanning')) && (
               <div className="absolute inset-0 z-30 pointer-events-none overflow-hidden">
                 ...
                 <div className="absolute inset-0 flex items-center justify-center">
                    <div className={`w-[25%] aspect-square border-2 ${themeClass} rounded-[40px] animate-pulse duration-75`} />
                 </div>
               </div>
            )}
            
            {/* Holographic Overlays for Success/Error */}
            {(!status.includes('ready') && !status.includes('holding') && !status.includes('scanning') && !showPinPad) && (
              <div className="absolute inset-0 z-40 backdrop-blur-sm bg-black/60 flex flex-col items-center justify-center animate-in zoom-in-95 duration-200">
                <div className={`w-full max-w-lg p-10 text-center border-y-2 bg-black/80 shadow-[0_0_40px_rgba(0,0,0,0.8)] ${
                   status.includes('success') ? 'border-[#00ff9d] text-[#00ff9d]' : 
                   status === 'duplicate' || status === 'wrong_group' ? 'border-[#ffb000] text-[#ffb000]' :
                  ...
                  <div className="mx-auto w-20 h-20 rounded-full flex items-center justify-center mb-6 shadow-xl bg-current/10 border border-current">
                    {status.includes('success') ? <UserCheck size={40} className="text-current" /> : 
                     status === 'duplicate' ? <ShieldAlert size={40} className="text-current" /> :
                     status === 'wrong_group' ? <UserX size={40} className="text-current" /> :
            ...
            {showPinPad && (
               <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center z-50 animate-in fade-in zoom-in-95 duration-200">
                  <div className="bg-[#020617] p-8 border border-[#ffb000] shadow-[0_0_30px_rgba(255,176,0,0.3)] max-w-sm w-full">
                     <h3 className="text-[#ffb000] text-center font-bold text-lg tracking-widest mb-6 uppercase">SAISIE CODE ENSEIGNANT</h3>
                     
                     <div className="flex justify-center gap-4 mb-8">
                       {[0, 1, 2, 3].map(i => (
                         <div key={i} className={`w-4 h-4 rounded-full border-2 transition-all ${
                           pinInput.length > i ? 'bg-[#ffb000] border-[#ffb000] shadow-[0_0_10px_rgba(255,176,0,0.8)]' : 'border-slate-700 bg-transparent'
                         }`} />
                       ))}
                     </div>
                         ...
                         <button
                           ...
                           onClick={async () => {
                             if (key === 'C') setPinInput('');
                               ...
                               try {
                                 const res = await fetch(`${API}/api/sessions/${activeSession.id}/verify-pin`, {
                                   method: 'POST',
                                   headers: { 'Content-Type': 'application/json' },
                                 ...
                                 if (res.ok && data.status === 'success') {
                                    ...
                                    setTimeout(() => { setResult(null); setStatus('ready_teacher'); }, 3000);
                                 }
                           ...
                           className={`py-4 text-2xl font-mono border transition-all active:scale-95 ${
                             key === 'OK' ? 'bg-[#ffb000]/20 text-[#ffb000] border-[#ffb000] hover:bg-[#ffb000]/40' : 
                             key === 'C' ? 'bg-red-500/10 text-red-500 border-red-500/50 hover:bg-red-500/30' : 
      ...
      <div className="w-full max-w-5xl mt-6 px-4 flex items-center justify-between z-50">
        <div className="w-1/3">
           <p className={`text-[10px] font-bold tracking-[0.2em] uppercase mb-1.5 ${themeClass.split(' ')[0]} transition-colors duration-500`}>
             ÉTAT DU SCANNER
           </p>
           <div className="flex gap-1 h-1.5">
             {[...Array(20)].map((_, i) => (
               <div key={i} className={`flex-1 transition-colors duration-300 ${i < (progress / 5) ? themeBgClass : 'bg-slate-800'}`} />
             ))}
           </div>
        </div>
        ...
        <div className="w-1/3 text-center">
           {activeSession && (
             <div className="inline-block px-6 py-2 border border-slate-700 bg-black/50 text-white text-xs tracking-widest uppercase rounded">
               <span className={`${themeClass.split(' ')[0]} transition-colors duration-500`}>{activeSession.course_name}</span> | GRP {activeSession.group_name}
             </div>
           )}
        </div>
        ...
        <div className="w-1/3 text-right">
           <div className="flex flex-col items-end gap-1">
             ...
             <p className="text-slate-500 text-[10px] tracking-[0.1em] uppercase leading-tight">
               MOTEUR IA: V2.4 | LATENCE: 12MS<br/>
