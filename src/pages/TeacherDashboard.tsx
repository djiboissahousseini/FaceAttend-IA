...
import {
  BookOpen, Calendar, Users, TrendingUp, WifiOff,
  RefreshCw, Server, Database, Clock, Zap, CheckCircle2,
  AlertCircle, Activity, GraduationCap, Unlock, LogOut,
  ShieldCheck, MonitorPlay, X, ShieldAlert, Key, Lock
} from 'lucide-react';
...
export default function TeacherDashboard() {
  ...
  const [securityInfo, setSecurityInfo] = useState<{is_blocked: boolean, failed_attempts: number, password: string} | null>(null);
  ...
  const [conn, setConn] = useState<ConnectionStatus>({
    api: 'checking', db: 'unknown', latency: null, lastChecked: null,
  });
  ...
  const toggleBlock = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/teachers/${loggedInTeacher.id}/toggle-block`, { method: 'POST' });
  ...
  const resetPass = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/teachers/${loggedInTeacher.id}/reset-password`, { method: 'POST' });
      if (res.ok) {
  ...
  const checkConnection = useCallback(async () => {
    ...
    try {
      ...
      setConn({
        api: 'offline', db: 'unknown', latency: null,
        lastChecked: new Date().toLocaleTimeString('fr-FR'),
      });
    }
  }, []);
  ...
  if (authLoading) {
    return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-cyan-400 font-mono tracking-widest animate-pulse uppercase">Initialisation du Portail...</div>;
  }
        ...
        <div className="min-h-screen bg-[#050a10] flex items-center justify-center font-mono p-4">
          <div className="w-full max-w-md bg-slate-900 border border-[#00f0ff]/30 p-8 shadow-[0_0_50px_rgba(0,240,255,0.1)]">
            <div className="flex flex-col items-center mb-8">
              ...
              <h1 className="text-xl font-bold text-white tracking-widest uppercase">Simulation Admin</h1>
              <p className="text-[#00f0ff]/60 text-[10px] mt-1 tracking-widest text-center uppercase">Mode Inspection : Sélectionnez un Profil</p>
            </div>
      ...
      <div className="min-h-screen bg-[#020617] flex items-center justify-center font-mono p-4">
        <div className="w-full max-w-md bg-slate-900 border border-blue-500/30 p-8 shadow-[0_0_50px_rgba(59,130,246,0.1)]">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-xl font-bold text-white tracking-widest uppercase">Portail Académique</h1>
            <p className="text-blue-400/60 text-[10px] mt-1 tracking-widest uppercase">Espace Enseignant Sécurisé</p>
          </div>
          ...
          <form onSubmit={handleTeacherLogin} className="space-y-5">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Mot de Passe</label>
              <input 
                type="password"
                value={loginPass}
          ...
          <p className="text-slate-600 text-[10px] mt-8 text-center leading-relaxed">
            Ce portail est exclusivement réservé au corps enseignant.<br/>
  ...
  const StatusDot = ({ status }: { status: 'online' | 'offline' | 'checking' | 'unknown' }) => {
    ...
    if (status === 'online') return <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse inline-block" />;
  ...
  const StatCard = ({
    ...
    color: 'blue' | 'green' | 'purple' | 'orange'; subtitle?: string;
  }) => {
    const colors = {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pt-4">
          <div className="flex items-center gap-4">
             <img 
               src={loggedInTeacher?.photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(loggedInTeacher?.name || 'User')}&background=020617&color=00f0ff`} 
               alt={loggedInTeacher?.name || 'User'}
               className="w-16 h-16 rounded-2xl shadow-md border-2 border-slate-200"
             />
             <div>
               <h1 className="text-2xl font-bold text-slate-800">Bienvenue, Pr. {loggedInTeacher?.name}</h1>
               <p className="text-slate-500 text-sm mt-0.5">{loggedInTeacher?.email}</p>
             </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => { checkConnection(); fetchData(); }}
        ...
        <div className="bg-slate-900 rounded-2xl p-5 grid grid-cols-1 sm:grid-cols-3 gap-4 shadow-lg">
          <div className="sm:col-span-3 flex items-center gap-2 mb-1">
            ...
            <h2 className="text-white text-sm font-bold uppercase tracking-widest">Diagnostic Système & Reconnaissance IA</h2>
          ...
          <div className="bg-slate-800 rounded-xl p-4 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${conn.api === 'online' ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
              <Server size={18} className={conn.api === 'online' ? 'text-emerald-400' : 'text-red-400'} />
            </div>
          ...
          <div className="bg-slate-800 rounded-xl p-4 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${conn.db === 'online' ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
              <Database size={18} className={conn.db === 'online' ? 'text-emerald-400' : 'text-red-400'} />
            </div>
            <div className="flex-1 min-w-0">
          ...
          {conn.api === 'offline' && (
             <div className="sm:col-span-3 text-xs text-red-400 mt-2">Le backend d'intelligence artificielle est déconnecté. Le système ne reconnaîtra pas les étudiants.</div>
          )}
        </div>
        ...
        {localStorage.getItem('faceattend_role') === 'admin' && securityInfo && (
          <div className="bg-slate-900 border-2 border-amber-500/30 rounded-2xl p-5 grid grid-cols-1 sm:grid-cols-3 gap-4 shadow-[0_0_30px_rgba(245,158,11,0.15)] animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="sm:col-span-3 flex items-center gap-2 mb-1">
              ...
              <h2 className="text-white text-sm font-bold uppercase tracking-widest">Contrôle de Sécurité & Accès</h2>
              <span className="ml-auto text-amber-500/60 text-[10px] font-mono font-bold px-2 py-0.5 bg-amber-500/10 border border-amber-500/20 rounded">MODE INSPECTION ADMIN</span>
            </div>

            <div className="bg-slate-800 rounded-xl p-4 flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${securityInfo.is_blocked ? 'bg-red-500/20' : 'bg-emerald-500/20'}`}>
                {securityInfo.is_blocked ? <Lock size={18} className="text-red-400" /> : <Unlock size={18} className="text-emerald-400" />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-white text-xs font-bold">Statut du Compte</p>
                <p className={securityInfo.is_blocked ? 'text-red-400 text-[10px]' : 'text-emerald-400 text-[10px]'}>
            ...
            <div className="bg-slate-800 rounded-xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-400 text-[10px]">{securityInfo.failed_attempts} tentative(s)</p>
              </div>
              <div className="flex gap-1">
                 {[...Array(3)].map((_, i) => (
                   <div key={i} className={`w-1.5 h-1.5 rounded-full ${i < securityInfo.failed_attempts ? 'bg-red-500 animate-pulse' : 'bg-slate-600'}`} />
                 ))}
              </div>
            </div>
            ...
            <div className="bg-slate-800 rounded-xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-400 text-[10px] font-mono tracking-wider">{securityInfo.password}</p>
              </div>
              <button 
                onClick={resetPass}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <span className="ml-auto text-xs bg-emerald-100 px-2 py-0.5 rounded-lg text-emerald-600 font-bold">{myTodaySessions.length}</span>
            </div>
            <div className="divide-y divide-slate-50">
              {myTodaySessions.length === 0 ? (
                <p className="text-slate-400 text-sm text-center py-8">Aucune séance prévue pour vous aujourd'hui.</p>
              ) : (
                  ...
                  <div key={session.id} className="flex items-center justify-between px-5 py-4 hover:bg-slate-50 transition-colors">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                         Groupe <span className="font-bold">{session.group_name}</span> • {session.classroom || 'Salle inconnue'}
                      </p>
                    </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2 bg-slate-50/50">
              ...
              <span className="ml-auto text-xs bg-slate-200 px-2 py-0.5 rounded-lg text-slate-600 font-bold">{myCourses.length}</span>
            </div>
            <div className="divide-y divide-slate-50">
              {myCourses.length === 0 ? (
                <p className="text-slate-400 text-sm text-center py-8">Vous n'avez aucun cours assigné dans la base.</p>
              ) : (
                myCourses.map((course) => (
                  <div key={course.id} className="flex items-center justify-between px-5 py-4 hover:bg-slate-50 transition-colors">
                    <div className="min-w-0">
                      <p className="font-medium text-slate-800 text-sm">{course.name}</p>
                      <p className="text-xs text-slate-400 mt-1">Code: {course.course_code} • Salle: {course.room || 'Non assignée'}</p>
                    </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden lg:col-span-2">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2 bg-slate-50/50">
              ...
              <span className="ml-auto text-xs bg-slate-200 px-2 py-0.5 rounded-lg text-slate-600 font-bold">{mySessions.length}</span>
            </div>
            <div className="divide-y divide-slate-50">
              {mySessions.length === 0 ? (
                <p className="text-slate-400 text-sm text-center py-8">Aucune séance enregistrée pour vos cours.</p>
              ) : (
